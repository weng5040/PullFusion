...
func NewConnectProxy(cfg *config.Config, deps *Dependencies) *http.Server {
	...
	adminAPI.SetDB(deps.NodeMgr.GetDB())
	r.Get("/healthz", healthzHandler(deps))
	r.Get("/dashboard", adminAPI.ServeDashboard)
	r.Get("/", adminAPI.ServeDashboard)
	r.Get("/dbconsole", adminAPI.ServeDBConsole)
	r.Get("/admin/db/tables", adminAPI.DBTables)
	r.Get("/admin/db/schema/{table}", adminAPI.DBSchema)
	r.Get("/admin/db/data", adminAPI.DBData)
	r.Post("/admin/db/insert", adminAPI.DBInsert)
	r.Post("/admin/db/update", adminAPI.DBUpdate)
	r.Post("/admin/db/delete", adminAPI.DBDelete)
